...
int main()
{
    ...
    do {
        ...
        case 2:
            ...
            int row, column; 
        cout<<"\n=====Hint Call=====";
        cout<<"\nEnter the row (1-9) you want the hint for: ";
        cin>>row;
            if (row < 1 || row > 9) { //checks row entry
                cout << "Invalid row input. No such cell exists\n";
                break;
            }
            cout<<"\nEnter the column (1-9) you want the hint for: ";
            cin>>column;
    ...
    for (int i = 0; i < 9; i++)
    {
        for (int j = 0; j < 9; j++)
        {
            if (rand() % 3 == 0) //if this gives 0, enter the number in that cell.
            {
                ...
                //  Mark this cell as fixed in the 
...
void hint(int arr[9][9], int row, int column, int&score){
    ...
    if (fixed_positions[hint_row][hint_column] == 1) {
        cout << "Cell (" << row << "," << column << ") is already fixed and cannot be changed." << endl;
        return;
    }
    
    if (arr[hint_row][hint_column] != 0) {
    ...
    do 
    {
        number = rand() % 9 + 1; // Generate a random number from 1 to 9 till it satisfies the valid condition.
    } while (!isValid(arr, hint_row, hint_column, number));

    // Output the number as hint
    arr[hint_row][hint_column] = number; 
    score -= 10; 
    //tell the user where hint is placed
    cout << "\nHint is placed at (" << row << "," << column << ") with number: " << number << endl;
    cout << "Score is reduced by 10 points.\n";
